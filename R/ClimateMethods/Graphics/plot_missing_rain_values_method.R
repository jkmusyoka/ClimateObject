...
climate$methods(plot_missing_values_rain = function(data_list=list(), threshold = 0.85, main = "Rain Present",xlab = "Year",legend_position="topright",
                                                    ylab = "Day of Year",station_name = TRUE, fill_col=c("blue","yellow","red")){    
  ...
  for(data_obj in climate_data_objs_list) {
    ...
    for( curr_data in curr_data_list ) {
      ...
      plot(curr_data[[ season_col ]],curr_data[[ dos_col ]], xlim = c(range(min(curr_data[[ season_col ]]), max(curr_data[[ season_col ]]))), 
           ylim = c(0,500), log = "", asp = NA, xlab = xlab, ylab =ylab, main = c(data_name,main))
      legend(legend_position,c("Rain","Dry","NA"),fill = fill_col, horiz=TRUE)
