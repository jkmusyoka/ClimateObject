...
#' 
climate$methods(plot_compare_longterm_tendaily_totals = function(data_list = list(), time_period="10-daily", year=1960,type=type, lty=lty, lwd=lwd,
...
#Get the data frames for analysis
    ...
    for( curr_data in curr_data_list ) {
      ...
      #subset by year.
      curr_data<-subset(curr_data, select=(year))
	...
	curr_data1[[Year]]<-year(curr_data1[[date_col]])
	curr_data1[[Month]]<-month(curr_data1[[date_col]])
	m_tot<-ddply(curr_data1,.(Year,Month),summarise,sum(as.numeric(Rain),na.rm=T))
